...
def evaluate_model(model, test_loader, device):
    ...
    return {
        ...
        'f1_score': f1,
        'roc_auc': roc_auc,
        'attention_weights': None  # MLP没有注意力机制
    }
...
def main():
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(X)):
        ...
        # 初始化模型 - 使用MLP而不是TabTransformer
        model = MLP(input_dim=input_dim, num_classes=num_classes, hidden_dims=[128, 64], dropout=0.1).to(device)
        ...
        # 保存模型和评估结果
        model_save_path = f'./models/mlp_fold_{fold+1}_f1_{fold_result["f1_score"]:.4f}.pth'
        torch.save({
