...
# Self Write Wrapper to use the TabTransformer in our code
class TabTransformerWrapper(nn.Module):
    def __init__(self, cat_dims, cat_idxs, num_idxs, num_classes=3, 
                 num_heads=4, num_layers=2, dim_model=64, dim_ff=128, dropout=0.2):
        super(TabTransformerWrapper, self).__init__()
    ...
    def forward(self, x):
        ...
        x_categ = x[:, self.cat_idxs].long()
        x_cont = x[:, self.num_idxs].float()
    ...
    def get_attention_weights(self, x):
        ...
        # Send them to the model and get attention weights
        _, attns = self.model(x_categ, x_cont, return_attn=True)
        # attns is a list of attention weights from each layer
        return attns[-1]
